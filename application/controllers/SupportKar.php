...
class SupportKar extends CI_Controller
{
    ...
    function index()
    {
        ...
        $data['support'] = $this->Support_model->getByUser();
    ...
    function edit($id)
    {
        ...
        if ($this->form_validation->run() == false) {
            $this->load->view("layout/header", $data);
            $this->load->view("lapor/vw_detail_support", $data);
            $this->load->view("layout/footer", $data);
        } else {
            $data = [
    ...
    function hapus($id)
    {
        ...
        $this->session->set_flashdata('message', '<div class="alert alert-success" role="alert">Laporan telah dianggap selasai!</div>');
        redirect('SupportKar');
    }
}
